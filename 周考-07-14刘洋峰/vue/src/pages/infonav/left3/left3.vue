...
let arr3 = ref([])
    ...
    const active = (p) => {
        ...
        router.push({
            path:'/infonav',
            query:{
                arr3: JSON.stringify(arr3.value)
            }
        })
...
<template>
    ...
    <div class="maininfo" style="flex: 1;">
        <van-grid :column-num="1" class="rightinfo" v-for="(p,m) in list.list" :key="p" style="width: 90px;height: 100px;">
    ...
    <footer>
        <div class="endbox">
            ...
            <div class="end">
                <p v-for="(item,index) in arr3" :key="item" class="iminfo">{{ item.name }} <button class="del">x</button> </p>
            </div>
        </div>
    </footer>
</template>
